...
# python -m train.train_featurizer_new_checkpoint_jason --exp dropout --lr 1e-3 --num_per_convo 128 --batch_size 512 --embedding_size 512
...
data_path = "/raid/lingo/leejason/automatic-annotation/dataset/original_data/standardized_dataset"
...
hard_noise_train = pickle.load(open(os.path.join(data_path, 'train_noise.p'), "rb"))
hard_noise_val = pickle.load(open(os.path.join(data_path, 'val_noise.p'), "rb"))
hard_noise_test = pickle.load(open(os.path.join(data_path, 'test_noise.p'), "rb"))
...
if __name__ == '__main__':
    ...
    scheduler_whale_embedding = StepLR(optimizer_whale_embedding, step_size=50, gamma=0.25) #default step = 50, gamma = 0.5
    ...
    for epoch in range(args.epoch):
        ...
        for i_batch, sample_batched in enumerate(train_dataloader):
            ...
            confidence_out = confidence_model(mlp_out)
            click_time_out = click_time_model(mlp_out)
            whale_embedding_out = whale_embedding_model(mlp_out)

            confidence_loss = criterion_confidence(confidence_out, confidence)
            confidence_loss = confidence_loss*2
            ...
            # click_time_loss = click_time_loss*4
            ...
            #to_compare = whale_embedding_out[0:number_per_conversation] #edit
            ...
            # loss = (confidence_loss + click_time_loss + whale_embedding_loss)
            ...
            avg_train_loss_all.append(confidence_loss.data.item() + click_time_loss.data.item() + whale_embedding_loss.data.item()*2.0)
            avg_train_loss_confidence.append(confidence_loss.data.item())
            avg_train_loss_click_time.append(click_time_loss.data.item())
            avg_train_loss_whale_embedding.append(whale_embedding_loss.data.item()*2.0)
        ...
        print(f'    Confidence loss: {np.mean(avg_train_loss_confidence)}, Click time loss: {np.mean(avg_train_loss_click_time)}, Whale embedding loss: {np.mean(avg_train_loss_whale_embedding)}')
        ...
        for i_batch, sample_batched in enumerate(val_dataloader):
            ...
            avg_val_loss_all.append(confidence_loss.data.item() + click_time_loss.data.item() + whale_embedding_loss.data.item())
        ...
        print(f'    Confidence loss: {np.mean(avg_val_loss_confidence)}, Click time loss: {np.mean(avg_val_loss_click_time)}, Whale embedding loss: {np.mean(avg_val_loss_whale_embedding)}')
        ...
        wandb.log({'val_loss': val_loss, 'val_confidence_loss': np.mean(avg_val_loss_confidence), 'val_click_time_loss': np.mean(avg_val_loss_click_time), 'val_whale_embedding_loss': np.mean(avg_val_loss_whale_embedding)})
    ...
    #plot_precision_recall_curve(click_time, out, save_path)
