...
##################################################
# Training command
# CUDA_VISIBLE_DEVICES=0,1,2,3 python train_transformer.py --exp 08_08_22_improved_featurizer_gpu_test --lr 5e-5 --epoch 200 --samp 10
...
parser.add_argument('--samp', type=int, default=5, help='Sample per update')
...
train_conversations = pickle.load(open('dataset/transformer_train_conversations_short.p',"rb"))
val_conversations = pickle.load(open('dataset/transformer_test_conversations_short.p',"rb"))
test_conversations = pickle.load(open('dataset/transformer_val_conversations_short.p',"rb"))
...
class sample_data(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        conversations = self.data_in[idx][:5000]
        num_included = 0
...
if __name__ == '__main__':
    ...
    featurizer_checkpoint = torch.load(os.path.join('featurizer', 'ckpts', '08_08_22_hard_noise', 'checkpoint.pth.tar'))
    ...
    # transformer_model = nn.DataParallel(transformer_model)    # !! Messed up the positional encoding, not sure how to fix.
    ...
    train_dataloader = DataLoader(train_dataset, batch_size=1, shuffle=True, num_workers=20)
    print('Number of training conversations: {}'.format(len(train_dataset)))

    val_dataset = sample_data(val_conversations)
    val_dataloader = DataLoader(val_dataset, batch_size=1, shuffle=False, num_workers=20)
    print('Number of validation conversations: {}'.format(len(val_dataset)))

    test_dataset = sample_data(test_conversations)
    test_dataloader = DataLoader(test_dataset, batch_size=1, shuffle=False, num_workers=20)
    ...
    for epoch in range(args.epoch):
        ...
        for sample_batched in train_dataloader:
            ...
            if i_batch%samples_per_epoch == 0:
            ...
            transformer_out = transformer_model(transformer_input, data_mask, low_confidence_mask)
            transformer_out = transformer_out[0]
            confidence_out = confidence_model(transformer_out)
            click_time_out = click_time_model(transformer_out)
            whale_embedding_out = whale_embedding_model(transformer_out)
            ...
            if confidence_out.shape[0] != 0:
                if confidence_loss is None:
                    confidence_loss = (criterion_confidence(confidence_out, masked_confidence))/samples_per_epoch
                else:
                    confidence_loss += (criterion_confidence(confidence_out, masked_confidence))/samples_per_epoch
            del confidence_out
            del masked_confidence

            mask = masked_whale_number.ge(1)
            click_time_out = click_time_out.flatten()
            click_time_out_masked = torch.masked_select(click_time_out, mask)
            click_time_masked = torch.masked_select(masked_click_time, mask)
            if click_time_masked.shape[0] != 0:
                if click_time_loss is None:
                    click_time_loss = (criterion_click_time(click_time_out_masked, click_time_masked)/60)/samples_per_epoch
                else:
                    click_time_loss += (criterion_click_time(click_time_out_masked, click_time_masked)/60)/samples_per_epoch
            del click_time_out
            ...
            actual_whale_number = masked_whale_number[mask]
            actual_whale_embedding_out = whale_embedding_out[mask]
            if actual_whale_embedding_out.shape[0] > 1:
                if whale_embedding_loss is None:
                    ...
                    whale_embedding_loss += (criterion_whale_embedding(actual_whale_embedding_out.unsqueeze(1), actual_whale_number)/4)/samples_per_epoch
                if math.isnan(whale_embedding_loss.data.item()):
                    print('Number of clicks: {}, Max whale number: {}, Min whale number: {}'.format(len(actual_whale_number), max(actual_whale_number), min(actual_whale_number)))
                    raise ValueError('Problem with whale embedding loss...')
            ...
            if (i_batch+1)%samples_per_epoch == 0:
                ...
                # print('Time to do loss stuff: {}'.format(start_time - time.time()))

                # print(f'After losses, Total Allocated: {torch.cuda.memory_allocated(0)/(1024*1024)} MiB')
        ...
        for i_batch, sample_batched in enumerate(val_dataloader):
            ...
            confidence_loss = None
            if confidence_out.shape[0] != 0:
                confidence_loss = criterion_confidence(confidence_out, masked_confidence)
                confidence_out = confidence_out.cpu().data.numpy()
                confidence_out = np.argmax(confidence_out,axis=1)
                masked_confidence = list(masked_confidence.cpu().data.numpy())
                acc_temp = 0
                for i in range(confidence_out.shape[0]):
                    if masked_confidence[i] == confidence_out[i]:
                        acc_temp += 1
            ...
            click_time_masked = torch.masked_select(masked_click_time, mask)
            if click_time_masked.shape[0] != 0:
                click_time_loss = criterion_click_time(click_time_out_masked, click_time_masked)/60
            ...
            actual_whale_embedding_out = whale_embedding_out[mask]
            if actual_whale_embedding_out.shape[0] > 1:
                whale_embedding_loss = criterion_whale_embedding(actual_whale_embedding_out.unsqueeze(1), actual_whale_number)/4
            del actual_whale_number
            del actual_whale_embedding_out
            del whale_embedding_out
