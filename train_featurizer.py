...
# python -m train.train_featurizer --exp n_dim_10 --lr 1e-3 --num_per_convo 128 --batch_size 512 --embedding_size 20
...
if __name__ == '__main__':
    ...
    probability_checkpoint = torch.load("/raid/lingo/martinrm/automatic-annotation/data_augmentation/prob_model/both_old_and_new_data_additive_augmentation.pth.tar")
    ...
    for epoch in range(args.epoch):
        ...
        for i_batch, sample_batched in enumerate(train_dataloader):
            ...
            # print("SHAPES",featurizer_1_output.shape,soundnet_out.shape)
            ...
            avg_train_loss_all.append(confidence_loss.data.item() + click_time_loss.data.item() + whale_embedding_loss.data.item())
        ...
        for i_batch, sample_batched in enumerate(val_dataloader):
            ...
            to_compare = whale_embedding_out[0:number_per_conversation].unsqueeze(1)
            whale_embedding_losses = [criterion_whale_embedding(to_compare, whale_number[0:number_per_conversation])]
            for i in range(number_per_conversation+8, whale_embedding_out.shape[0], number_per_conversation+8):
                if i < whale_embedding_out.shape[0] - number_per_conversation:
                    to_compare = whale_embedding_out[i:i+number_per_conversation].unsqueeze(1)
                    to_add = criterion_whale_embedding(to_compare, whale_number[i:i+number_per_conversation])/(args.num_per_convo/4)
    ...
    # plot_precision_recall_curve(label, out, save_path)
