...
# python -m train.train_featurizer_gaussian --exp test_gaussian --lr 1e-3 --num_per_convo 128 --batch_size 512 --embedding_size 512
...
wandb.init(project="whale-featurizer2",
...
if __name__ == '__main__':
        ...
        def generate_data(self):
            ...
            for _ in range(self.num_samples):
                if np.random.rand() > 0.5:
                    sample = np.random.normal(self.mean1, self.std1, size = (whale_embedding_size,))  # Generate a sample with 512 dimensions
                    label = 0  # Label 0 for the first Gaussian dataset
                else:
                    sample = np.random.normal(self.mean2, self.std2, size = (whale_embedding_size,))  # Generate a sample with 512 dimensions
    ...
    for epoch in range(args.epoch):
        ...
        for inputs, labels in train_dataloader:
            ...
            threshold = 0.8
            whale_embedding_losses = []
            for i in range(whale_embedding_out.shape[0]):
                for j in range(whale_embedding_out.shape[0]):
                    if i != j:
                        first = whale_embedding_out[i].unsqueeze(1).cpu().detach().numpy()
                        first_label = whale_number[i]
                        second = whale_embedding_out[j].unsqueeze(1).cpu().detach().numpy()
                        second_label = whale_number[j]
                        ...
                        distance = abs(np.linalg.norm(first-second))
                        #print(distance, ground)
                        if distance > threshold:
                            output = "diff"
                        else:
                            output = "same"

                        if first_label == second_label and output == "diff":
                            whale_embedding_losses.append(torch.tensor(1.0, requires_grad=True))
                        elif first_label != second_label and output == "same":
                            whale_embedding_losses.append(torch.tensor(1.0, requires_grad=True))
                        else:
                            whale_embedding_losses.append(torch.tensor(0.0, requires_grad=True))
            
            whale_embedding_loss = sum([individual_loss for individual_loss in whale_embedding_losses]) / len(whale_embedding_losses)
            ...
            # print(to_compare)
            # print(whale_number[:number_per_conversation])
